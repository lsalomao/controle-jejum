...
```bash
sudo mkdir -p /opt/fasting-life
sudo chown $USER:$USER /opt/fasting-life
cd /opt/fasting-life
```
...
scp -r * usuario@seu-vps-ip:/opt/fasting-life/
```

### Opção C: Usando rsync (da sua máquina local)
```bash
# Na sua máquina local, no diretório do projeto
rsync -avz --exclude 'db.sqlite3' --exclude '__pycache__' --exclude '*.pyc' --exclude '.git' . usuario@seu-vps-ip:/opt/fasting-life/
```
...
ALLOWED_HOSTS=fasting.ledtech.app,localhost,127.0.0.1
...
CSRF_TRUSTED_ORIGINS=https://fasting.ledtech.app
```
...
```bash
sudo cp /opt/fasting-life/sites-available/fasting.ledtech.app /etc/nginx/sites-available/fasting.ledtech.app
```

### 4.2 Editar configuração com seu domínio
```bash
sudo nano /etc/nginx/sites-available/fasting.ledtech.app
```

Substitua `fasting.ledtech.app` pelo seu domínio em todas as ocorrências.

### 4.3 Criar link simbólico
```bash
sudo ln -s /etc/nginx/sites-available/fasting.ledtech.app /etc/nginx/sites-enabled/
```
...
```bash
sudo certbot --nginx -d fasting.ledtech.app
```
...
```bash
sudo mkdir -p /opt/fasting-life/staticfiles
sudo mkdir -p /opt/fasting-life/media
docker cp fasting_web:/app/staticfiles/. /opt/fasting-life/staticfiles/
docker cp fasting_web:/app/media/. /opt/fasting-life/media/
sudo chown -R www-data:www-data /opt/fasting-life/staticfiles
sudo chown -R www-data:www-data /opt/fasting-life/media
```
...
```
http://fasting.ledtech.app
```
ou
```
https://fasting.ledtech.app (se configurou SSL)
```
...
```bash
docker-compose exec db pg_dump -U fasting_user fasting_db > backup_$(date +%Y%m%d_%H%M%S).sql
```

### Restaurar banco de dados
```bash
docker-compose exec -T db psql -U fasting_user fasting_db < backup.sql
```
...
- Verifique se a porta 4000 está acessível: `curl http://localhost:4000`

### Arquivos estáticos não carregam
- Verifique permissões: `ls -la /opt/fasting-life/staticfiles`
...
- **80**: HTTP (Nginx)
- **443**: HTTPS (Nginx)
- **4000**: Aplicação Django (apenas localhost)
